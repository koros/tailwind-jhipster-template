  ...
  return (
    <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 border-b border-gray-200">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium text-gray-900" id={id || 'modal-title'} data-cy={dataCy}>
          {children}
        </h3>
        {toggle && (
          <button type="button" className="text-gray-400 hover:text-gray-500 focus:outline-none" onClick={toggle}>
...
export const ModalBody: React.FC<ModalBodyProps> = ({ id, children }) => {
  return (
    <div id={id} className="bg-white px-4 pt-5 pb-4 sm:p-6">
      {children}
    </div>
  );
};
...
export const ModalFooter: React.FC<ModalFooterProps> = ({ children }) => {
  return <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">{children}</div>;
};
